def adjacent_positions(
    ...
    col_position: int,
    total_rows: int = 140,
    total_cols: int = 140,  # Subtract 1 for the newline
) -> set[tuple[int, int]]:
    ...
    Returns:
        A set of the adjacent (row, col) cells to the given cell.
    """
...
def forklifts_in_positions(
    ...
    Returns:
        The number of forklifts present in the given cells.
    """
    forklifts = 0

    for cell_row, cell_col in positions:
        if input_data[cell_row][cell_col] == "@":
            forklifts += 1
